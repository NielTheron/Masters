...
wy_p    = 0;                            % Yaw   rate B/O (deg/s)
wz_p    = 0;                            % Pitch rate B/O (deg/s) 
...
wy_f    = 0;                        % Yaw   rate B/O (deg/s)
wz_f    = 0;                        % Pitch rate B/O (deg/s)
...
for r = 1:n_s-1
    ...
    j = mod(t,dt_MAG);
    if j == 0
        z_MAG(:,r) = Magnetometer(x_true(1:3,r),x_true(7:10,r),noise_MAG,0,t,we_p);
    else
        z_MAG(:,r) = [0 0 0].';
    end
    %---
